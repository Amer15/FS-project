...
exports.forgotPassword = (req, res) => {
    ...
    User.findOne({ email }).exec((err, user) => {
        ...
        var mailOptions = {
            ...
            html: `
            <div>
            <h2 style="background-color: #10A881; color: white; text-align:center; padding:10px; font-family:Arial, Helvetica, sans-serif">Please use the following link to reset your password</h2>
            <br/>
    ...
    if (token) {
        return jwt.verify(token, process.env.JWT_RESET_PASSWORD_KEY, (err) => {
            ...
            User.findOne({ email }).exec((err, user) => {
                ...
                user.updateOne({ $set: { salt: salt, encrypted_password: newPassword } }).exec((err, user) => {
                    if (err) return res.status(400).json({
